...
class ShipLogicApi
{
    ...
    public function getRates(array $package, array $parameters): array
    {
        if ($wcSession = WC()->session) {
            ...
            if (!empty($package['insurance']) && $package['insurance']) {
                $insurance = true;
            } elseif (isset($postData['shipping_insurance']) || isset($postData['billing_insurance'])) {
                $insurance = true;
            }
            ...
            try {
                ...
                if ($enabledTcgLockers) {
                    $lockerRates = $this->getLockerRates(clone $body);
    ...
    private function getSender(array $parameters): object
    {
        ...
        $sender->street_address = $parameters['shopAddress1'] ?? '';
        $sender->local_area     = $parameters['shopSuburb'] ?? '';
